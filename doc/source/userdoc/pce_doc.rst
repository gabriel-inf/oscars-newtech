...
**5.   Reservation.**
	a. If Pathfinding is successful, save the selected network resources (route information, selected VLAN, schedule, etc.) into the circuit's :doc:`resvspec_doc`. Otherwise, fail gracefully.
	b. Save the circuit details in the Reservation Database. 

**6.   Configure the networks.**
	a. Forward the reservation details to the :doc:`pss_doc` for network configuration.
...
VLANService
Calls: ReservedVlanRepository
Called By: TopPCE, PruningService, TranslationPCE
Description: Provides methods that handle retrieving reserved VLAN IDs from the repository, mapping reserved/available/requested VLAN IDs to network elements, and determining if requested VLAN IDs can be supported on a topology.
